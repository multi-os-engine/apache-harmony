...
public class DisposeDuringInvokeTest extends JDWPSyncTestCase {
     ...
     * <BR>Then the test sets a breakpoint a tested thread (DebuggeeThread) gets suspended by an
     * event. Once this thread is suspended, we send an ObjectReference.InvokeMethod command
     * to it to initiate a method invocation in that thread. The method will synchronize with us,
     * waiting for a signal to continue its execution.
     * <BR>While the tested thread waits for the signal, we send a VirtualMachine.Dispose command
     * and sends the expected signal so the tested thread completes the method invocation.
